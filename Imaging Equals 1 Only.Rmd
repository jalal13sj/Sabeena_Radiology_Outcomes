...
# Summary
I don't think there is evidence of confounding from the measured covariates. Overall, there is at most a small effect on MD time due to the intervention. Some of the analyses don't support that there is a difference between the MD time pre and post intervention. There might be a stronger effect of the intervention in some subgroups, but the subgroup analysis also shows the intervention increaseing MD time in some subgroups, which is odd. There may be unmeasured confounding that is distorting the relationships. Below there is a summary and then each chunk of code has a short explanation. 

## Data
group:	The intervention, Pre 24/7 support, Post 24/7 support;	0= pre, 1= post
...
Note: The post intervention median outcome (Md time, mdex) was *higher* than the pre intervention median. The post intervention mean outcome was lower than the pre intervention mean. 
...
##Effect of group: 
The Mann-Whitney U test says that there is no effect due to group. Looking at the KM and other analyses, at most, the effect of group seems to be very small. It seem that the pre intervention group usually has a smaller median mdex, but that the post intervention gets "faster" after hitting the median (ie: it is quicker for "longer" cases). As an analogy, the pre intervention group is like a sprinter is quick and has the fastest 100 meter time and the post intervention group is slower out of the blocks, but wins the 400 meter race with a strong back half of the race. The intervention group is slower for mdex times under about ~400 minutes, and then it becomes faster.

Sub-group analysis: Intervention may increase the mdex (mean and median) time for trauma patients and patients that are not admitted. The intervention may decrease mdex time in admitted patients. There may be subgroup effects in CTAS and Todseen, but it's hard to say. These subgroup results are consistent across most methods. 
...
There are more very low values in the pre 24/7 group than in the post group. There are more moderately low values in the post group than in the pre group. If there is no confounding, then this suggests that the effect of the intervention switches (negative at very low mdex times and positive at moderately low mdex times). 
...
```
It doesn't look like a huge difference between the groups, but post looks a little higher.  
...
Side note: we can see the median and mean for each group. The post group has a lower mean but a higher median. 
...
There is no censoring and I don't think that there is confounding, so would could simply do a Mann-Witney U test between group = 0 and group = 1.
...
```
This gives a p value of 0.078, so we don't have enough evidence to reject the null that the populations are identical. We have a huge sample size, so it is not for lack of observations. If there is a difference between the populations, it's small. Recall the median mdex for the post 24/7 intervention (group = 1) is *higher* than the median for the pre 24/7 intervention group. The mean of the post 24/7 group is lower. This is a right skewed data set so the median is probably a better measure of center. I still need to do a subgroup analysis of this. 
...
```
When stratified by adm, the subgroups come from different populations (ie: there is an effect due to the intervetion). Recall the descriptive stats, median and mean mdex is *longer* in the post 24/7 group for patients that are not admitted. Median and mean mdex is shorter in the post 24/7 group for patients that are not admitted. 
...
```
When stratified by trauma, only trauma patients come from different populations (ie: there is an effect due to the intervention in the trauma subgroup). Non-trauma patients in both pre and post groups come from the same population. Median and mean mdex is *longer* in the post 24/7 group for trauma patients.
...
colnames(all.w.df) <- c("Subgroup", "M-W p value", "Measure of Center", "Pre 24/7", "Post 24/7", "Improvement?")
...
No effect on mdex due to intervention, though the means and medians suggest that the intervention may even increase mdex time. The clearest subgroup effects (p value < 0.05 and a change in mean and median in the same direction) are in Adm0, Adm1, and Trauma1. In Adm0 and Trauma1, the mean and median mdex time increased. In Adm1, the mdex time decreased. 
...
```
That was fun! The univariate model suggests that mdex is 2.7% to 5.5% faster in the intervention group. But before I get too excited, I should probably check the proportional hazards assumption **ominous music plays in the background**
...
```
You can see in the full KM graph that the red line dips below the blue line between roughly 500 and 1500 minutes. That means that in that region, the post 24/7 is faster. However when we zoom in to under 500 minutes, the blue line is below the red line, which means that the pre 24/7 is faster. This is compatible with the first couple of histograms at the start of this document where we saw that the pre 24/7 group has more observations with very small mdex. The mean for the post group is lower, but the median is higher. So from the graph, we can see that the first 50% of processes occur faster in the pre group, but then the post group gets faster. 
...
```
A quick visual inspection shows CTAS2 and CTAS3 have the clearest benefits of 24/7. CTAS1 has the switching of effect and CTAS4 and 5 are less clear. When you zoom in, you can see that most of them cross, with the pre group being lower under about 400 minutes and then the post group being lower after that. 

CTAS1: The survival curves cross. Pre is below for the most part for mdex < 1000 and then post is below for the most part after that. Post median and mean are lower. Lines cross and a p = 0.5

CTAS2: We can see post is below for probably the entire curve. Post median and mean are lower. p = 0.3

CTAS3: Between 100 and 250, pre is slightly below and then after 250, post is below. Post mean is lower, but post median is higher. p = 0.6 

CTAS4: Pre is slightly below until 250 and then post is slighty below after that. Post mean is lower, but post median is higher. p = 2e-05

CTAS5: Pre is slightly below from 50 until about 250, and then post is below after that. Post mean is lower, but post median is higher. p = 0.6

KM CTAS sub group analysis conclusion: The intervention may increase median mdex in CTAS3, CTAS4 and CTAS5 (may be conclusive for CTAS3). The intervention may reduce mdex time in CTAS1 and CTAS2. 
...
```
Not admitted (Adm = 0): Looks like pre is below for the entire curve. The post median and mean are both higher. p = <2e-16 

Admitted (Adm = 1): Looks like post is below for the entire curve. The post median and mean are both lower. p = <2e-16 

KM Adm sub group analysis conclusion: The intervention reduces median mdex in those who are admitted to the hospital (adm = 1). Otherwise, the intervention increases median mdex for those not admittd (adm = 0). 
...
table1(~ adm + todseen + mdex + ctas | trauma*group, data = table1_data_na)

```
Trauma = 0: Pre is below post until about 250 after which post is below. Post mean is lower and post median is higher. p = 0.8

Trauma = 1: Pre is below until about 500. It looks like post might get below after that, but not by much. Post mean and median are higher. p = 1e-07  

KM Trauma subgroup conclusion: It looks like the intervention increases the mdex time in trauma patients. Note: there is a much larger sample size for Trauma = 0. 
...
```
Todseen = 1: Pre is barely below between 50 and 250. Post is below around 300 to 2000. Post mean and median are lower. p = 0.1

Todseen = 2: Pre is below up to about 350 and post is below after that until about 2000. There's a weird hump in pre around 1000. Post mean is lower, but the post median is higher. p = 0.01

Todseen = 3: Post is below until about 350, after which pre is slightly below until a little past 200. Post mean is lower, but the post median is higher. p = 0.02

KM Todseen subgroup conclusions: The intervention may reduce median mdex in Todseen = 1, but is inconclusive. The intervention increases median mdex in Todseen = 2 and 3.

###Overall KM results

Very small differences in mdex between the groups. It seem that the pre intervention group usually has a smaller median mdex, but that the post intervention gets "faster" after hitting the median (ie: it is quicker for "longer" cases). The pre intervention group is the sprinter that has the fastest 100m and the post intervention group is slower out of the blocks, but wins the 400m with a strong back half of the race. The intervention is slower up until about ~400 minutes, and then it becomes faster.

Intervention may increase the mdex (mean and median) time for trauma patients and patients that are not admitted. The intervention may decrease mdex time in admitted patients. There may be subgroup effects in CTAS and Todseen, but it's hard to say.
...
```
With only group, the model describes the effect of group at -17 minutes (ie: 17 minutes faster in post 24/7). That's 3.2% to 5.9% shorter mdex after the intervention. This is basically a t-test comparing means. 
...
```
Shorter mdex times in post 24/7 group for non-trauma patient. Longer mdex times in post 24/7 group for trauma patients. This is compatible with what we saw in the KM subgroup analysis.
...
```
Shorter mdex times in post 24/7 group for todseen = 1.
...
summary(bic.linear)
```
"Full up" model has group effect at -11 minutes. The full model has a higher R (obvously), but recall prediction is not the aim. 
...
```
Now the 3 assumptions are much more respected (see the graphs), but the R2 has gone down. In this case, group doesn't seem to have an effect (the CI is 0.99, 1.02). 
...
```
Longer mdex times in post 24/7 group when patient not admitted. Shorter mdex times in post 24/7 group when patient admitted. This is compatible with what we saw in the KM  and linear regression subgroup analysis.
...
```
Maybe shorter mdex times in post 24/7 group for non-trauma patient, but inconclusive. Longer mdex times in post 24/7 group for trauma patients. This is compatible with what we saw in the KM  and linear regression subgroup analysis.
...
```
Shorter mdex times in post 24/7 group for todseend = 1 (morning). Longer for todseen = 2 and 3. 
...
g.uni.ord.cis
```
I think this is saying that the intervention makes it more likely to take longer (ie: be in a higher mdex tertile). This fits with what we've seen where the median is smaller for the pre 24/7 group. 
